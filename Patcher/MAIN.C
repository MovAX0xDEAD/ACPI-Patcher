...
static struct		e820range e820_ranges[MAXRANGES];
...
void e820_init(void) {
	...
	do {
		...
		reg.d.ecx = sizeof(struct e820_data);
		reg.x.es = __tb >> 4;
		reg.x.di = __tb & 0x0f;
		__dpmi_int (0x15, &reg);
...
acpi_header_t* find_in_acpi (const char *p_tablename, uint32_t *rsdt_ptr, uint32_t *xsdt_ptr, bool multi) {
	...
	for(i = 0; i < rsdt_entries && i < 100; i++) {
		entry=(acpi_header_t *) mapaddr((uint32_t) glb_rsdt->entry[i]);
			//LOG("entry=%08x \n", (uint32_t) glb_rsdt->entry[i]);
			//HEXLOG((char *)entry,"",32);
		if(memcmp(entry->signature, "FACP", 4) == 0) {
				//LOG("found facp in rsdt \n");
			if (glb_facp0 == NULL) {
				glb_facp0 = (acpi_fadt_t *) entry;
				glb_raw_facp0 = (uint32_t)glb_rsdt->entry[i];
				if (glb_raw_facp0 < raw_base || glb_raw_facp0 > (raw_base+raw_size)) {printf("ACPI Patcher Error: FACP addr is in diff reserved memory\n"); err_exit(1);}
			...
			if(memcmp(p_tablename, "DSDT", 4) == 0) {
				...
				if (raw_dsdt < raw_base || raw_dsdt > (raw_base+raw_size)) {printf("ACPI Patcher Error: DSDT addr is in diff reserved memory\n"); err_exit(1);}
				*rsdt_ptr = raw_dsdt;
				ret = (acpi_header_t *) mapaddr(raw_dsdt);
				break;}
			if(memcmp(p_tablename, "FACS", 4) == 0) {
				raw_facs = (uint32_t) glb_facp0->facs_addr;
				if (raw_facs < raw_base || raw_facs > (raw_base+raw_size)) {printf("ACPI Patcher Error: FACS addr is in diff reserved memory\n"); err_exit(1);}
				*rsdt_ptr = raw_facs;
		...
		if(memcmp(entry->signature, p_tablename, 4) == 0)
		{
			if (indx != -1) {
				...
				dub_rsdt[dub_i].rsdt_ptr=(uint32_t) glb_rsdt->entry[i];
				if (dub_i > dub_max) dub_max=dub_i;
				continue;
			}
			if ((uint32_t)glb_rsdt->entry[i] < raw_base || (uint32_t)glb_rsdt->entry[i] > (raw_base+raw_size)) {printf("ACPI Patcher Error: %s addr is in diff reserved memory\n",p_tablename); err_exit(1);}
			*rsdt_ptr = (uint32_t) glb_rsdt->entry[i];
			ret = entry;
			break;
		}
	}
	...
	if (dub_i) {
		...
		if (multi == false) {
			...
			if (dub_rsdt[indx].rsdt_ptr < raw_base || dub_rsdt[indx].rsdt_ptr > (raw_base+raw_size)) {printf("ACPI Patcher Error: %s addr is in diff reserved memory\n",p_tablename); err_exit(1);}
	...
	for(i = 0; i < xsdt_entries && i < 100; i++) {
		entry=(acpi_header_t *) mapaddr((uint32_t) glb_xsdt->entry[i]);
			//LOG("entry=%08x \n", (uint32_t) glb_xsdt->entry[i]);
			//hexdump((char *)entry,"",32);
		if(memcmp(entry->signature,"FACP", 4) == 0)
		{
				//LOG("found facp in xsdt \n");
			if (glb_facp1 == NULL) {
				glb_facp1 = (acpi_fadt_t *) entry;
				glb_raw_facp1 = glb_xsdt->entry[i];
				if (glb_raw_facp1 < raw_base || glb_raw_facp1 > (raw_base+raw_size)) {printf("ACPI Patcher Error: FACP addr is in diff reserved memory\n"); err_exit(1);}
			...
			if(memcmp(p_tablename, "DSDT", 4) == 0) {
				...
				if (raw_dsdt < raw_base || raw_dsdt > (raw_base+raw_size)) {printf("ACPI Patcher Error: DSDT addr is in diff reserved memory\n"); err_exit(1);}
				*xsdt_ptr=raw_dsdt;
				if (ret != (acpi_header_t*) mapaddr(raw_dsdt)) { printf("DSDT is diff in rsdt and xsdt, ignoring rsdt \n");}
			...
			if(memcmp(p_tablename, "FACS", 4) == 0) {
				...
				if (raw_facs < raw_base || raw_facs > (raw_base+raw_size)) {printf("ACPI Patcher Error: FACS addr is in diff reserved memory\n"); err_exit(1);}
				*xsdt_ptr=raw_facs;
				if (ret != (acpi_header_t*) mapaddr(raw_facs)) { printf("FACS is diff in rsdt and xsdt, ignoring rsdt \n");}
		...
		if(memcmp(entry->signature, p_tablename, 4) == 0)
		{
			if (indx != -1) {
				...
				dub_xsdt[dub_i].xsdt_ptr=(uint32_t) glb_xsdt->entry[i];
				if (dub_i > dub_max) dub_max=dub_i;
				continue;
			}
			if ((uint32_t)glb_xsdt->entry[i] < raw_base || (uint32_t)glb_xsdt->entry[i] > (raw_base+raw_size)) {printf("ACPI Patcher Error: %s addr is in diff reserved memory\n",p_tablename); err_exit(1);}
			*xsdt_ptr = glb_xsdt->entry[i];
	...
	if (dub_i) {
		...
		if (multi == false) {
			...
			if (dub_xsdt[indx].xsdt_ptr < raw_base || dub_xsdt[indx].xsdt_ptr > (raw_base+raw_size)) {printf("ACPI Patcher Error: %s addr is in diff reserved memory\n",p_tablename); err_exit(1);}
...
void upd_rsdp () {
	...
	uint8_t			chksum;
		...
		LOG("Update RSDP \n");
	if (glb_raw_rsdp <= (uint32_t)glb_ebda || glb_raw_rsdp >= (uint32_t)glb_ebda_end) { // out of ebda
	...
	else new_rsdp=(acpi_rsdp_v20_t *) glb_raw_rsdp;
...
void upd_rsdt_xsdt(tree_t *node) {
 ...
 if (rsdt_moved == 0)
	if (rawmem_in_bios_rom(glb_raw_rsdt) || rawmem_is_readonly(map_ds, glb_raw_rsdt - raw_base, 1)) {
 ...
 if (xsdt_moved == 0 && acpi_revision > 1)
	if (rawmem_in_bios_rom(glb_raw_xsdt) || rawmem_is_readonly(map_ds, glb_raw_xsdt - raw_base, 1)) {
	...
	for(i = 0; i < rsdt_entries && i < 100; i++) {
		...
		if(memcmp(entry->signature, node->name, 4) == 0 && (uint32_t) glb_rsdt->entry[i] == node->rsdt_ptr) {
				LOG("sig == name %s == %s  entry == rsdt_ptr %08x == %08x\n", entry->signature, node->name, (uint32_t) glb_rsdt->entry[i], (uint32_t) node->rsdt_ptr);
...
void upd_facp(tree_t *node) {
	...
	if (glb_facp0 == NULL) {printf("ACPI Patcher Error: facp0 is not filled\n"); err_exit(1);}
	//if (glb_facp0->header.length  > 116) facp0_ver =2;
	if (glb_facp1) {
	...
	if (facp0_moved == 0)
		 if (rawmem_in_bios_rom(glb_raw_facp0) || rawmem_is_readonly(map_ds, glb_raw_facp0 - raw_base, 1))  {
	...
	if (facp1_ver && facp1_moved == 0)
		 if (rawmem_in_bios_rom(glb_raw_facp1) || rawmem_is_readonly(map_ds, glb_raw_facp1 - raw_base, 1))  {
...
void difpatch (const char *p_tablename, const char *filename) {
 ...
 if (p_table) {
	...
	*/
	...
	if 	(!memcmp(compiler_name, "INTL", 4) && compiler_ver <= 0x20161231)
...
int main (int argc, char *argv[]) {
 ...
 //ROOT_TREE->next=0;
 //ROOT_TREE->changed=0;
 //ROOT_TREE->allocated=0;
 //ROOT_TREE->newstore=0;
 //ROOT_TREE->ptr=0;
